...
impl EnvDetector for GitDetector {
    ...
    fn detect(&self) -> Result<Option<RuntimeEnv>> {
        ...
        let version_str = run_command(&git_path.to_string_lossy(), &["--version"]);
        ...
        if let Some(user_name) = run_command("git", &["config", "user.name"]) {
            mirrors.insert("user.name".to_string(), user_name);
        }
        if let Some(user_email) = run_command("git", &["config", "user.email"]) {
        ...
        Ok(Some(RuntimeEnv {
            name: "git".to_string(),
            version,
            install_path: git_path.to_string_lossy().to_string(),
            mirrors,
