...
[SECURITY]
[SECURITY] OneInit 将从网络download文件、修改 PATH 环境变量、
[SECURITY] 写入配置文件、并可能execute install脚本。
[SECURITY]
[SECURITY] 请确认你信任recipe的来源和download地址。
[SECURITY] 社区recipe未经审计，使用风险自负。
[SECURITY] ============================================";
...
[SECURITY]   1. 从网络download文件到 ~/.oneinit/ 目录
[SECURITY]   2. 修改系统 PATH 环境变量
[SECURITY]   3. 写入配置文件 (如 pip.conf, .npmrc)
[SECURITY]   4. 可能execute install脚本 (如 get-pip.py)
[SECURITY]
[SECURITY] 安全建议:
[SECURITY]   - 仅安装你信任来源的recipe
[SECURITY]   - 安装前查看 [SECURITY] 提示的download地址和命令
[SECURITY]   - 使用 oneinit verify 验证社区recipe
