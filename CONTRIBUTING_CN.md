...
| 提问 / 求助 | GitHub Issues（标 `Question` 标签） |
...
2. 说清楚两件事：
   ...
   - **建议怎么做**（方案、接口、示例）
3. 先看看 [ROADMAP_CN.md](ROADMAP_CN.md) —— 你的想法可能已经在规划中，或与现有方向冲突，先讨论再动手
...
4. **审核流程**：
   - 改动 `/src`、`Cargo.toml`、`install.sh` 需要 `@oneinitAI` **代码所有者**批准
   - `www/`、文档、`.agents/skills/`、`.github/` 等路径**无代码所有者、免 code owner 审核**，
     但**仍需仓库管理者（维护者）审核通过后才能合并**——不会自动合并
5. 合并采用 **squash**（历史干净）
...
- **错误处理**：统一用 `CoreError` + `Result`（定义在 `src/core/mod.rs`）；不要到处 `unwrap()`，
  除非在测试里或确实不可能失败的地方
- **注释**：**英文**。注意：现有代码仍是中文注释——项目正在逐步迁移到英文，
  **新代码必须使用英文注释**；公共 API（`pub fn`）加 `///` 文档注释
...
- **安全**：任何涉及下载、执行命令、写文件的改动，都要遵守项目现有的安全模型
  （SHA256 校验、`--allow-exec` 默认拒绝、签名验证）
...
**Q：我的 PR 为什么显示"无需 code owner 审核"？**
改动只涉及 `www/`、文档、`.agents/`、`.github/` 等没有代码所有者的路径时，
跳过 code owner 批准步骤。但**仍需 CI 通过 + 仓库管理者审核合并**——不会自动合并。
...
如果已被分配，就换一个 —— 仓库会保持 3-5 个未分配的 Good First Issues。
...
项目的目标和发展方向见 **[ROADMAP_CN.md](ROADMAP_CN.md)**。
